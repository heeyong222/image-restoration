...
def create_generator(opt):
    ...
    # summary(generator, input_data=[dummy_input, dummy_input])
    if opt.load_name == '':
        # Init the network
...
def load_dict(process_net, pretrained_net):
    # pretrained_net은 이미 load된 state_dict (dictionary)라고 가정합니다.
    pretrained_dict = pretrained_net
    process_dict = process_net.state_dict()
    
    # process_net에 존재하는 키 중, pretrained에서 불러올 수 있는 키만 필터링
    loaded_dict = {k: v for k, v in pretrained_dict.items() if k in process_dict}
    
    # 불러온 키와 불러오지 못한 키, 그리고 pretrained에만 존재하는 키 확인
    loaded_keys = set(loaded_dict.keys())
    process_keys = set(process_dict.keys())
    pretrained_keys = set(pretrained_dict.keys())
    
    missing_keys = process_keys - loaded_keys
    extra_keys = pretrained_keys - loaded_keys
    
    print("=== Pretrained 모델에서 불러온 레이어 ===")
    for k in loaded_keys:
        print("  ", k)
    
    print("\n=== 불러오지 못한 레이어 (현재 모델에 존재하지만 pretrained에 없음) ===")
    for k in missing_keys:
        print("  ", k)
    
    if extra_keys:
        print("\n=== pretrained 모델에만 존재하는 레이어 (불필요한 키) ===")
        for k in extra_keys:
            print("  ", k)
    
    # 필터링된 pretrained weight로 process_dict 업데이트
    process_dict.update(loaded_dict)
    process_net.load_state_dict(process_dict)
    return process_net
...
def get_files(path):
    ret = []
    path_rainy = path + "/train/input" # "/small/rain"
    path_gt = path + "/train/gt" # "/small/norain"
...
def get_val_files(path):
    ret = []
    path_rainy = path + "/val/input" # "/small/rain"
    path_gt = path + "/val/gt" # "/small/norain"
